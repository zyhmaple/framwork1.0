...
public class InterfaceConnentUtil {
	public static Document AcceptAndSendXmlFile(String httpUrl, String xmlStr) throws Exception {
		...
		try {
			try {
				...
				throw new Exception(
						"\u8fde\u63a5\u8fd4\u56de\u7684\u9519\u8bef\u4ee3\u7801\u4e3a\uff1a" + conn.getResponseCode());
			} catch (Exception e) {
				e.printStackTrace();
	...
	public static String sendURLAndGetXmlFile2Str(String httpNewUrl) throws Throwable {
	...
	public static String sendURLAndGetStr(String httpNewUrl, String encoding) throws Exception {
		...
		try {
			try {
				...
				String code = httpurlconnection.getHeaderField("Content-Encoding");
				input = code != null && "gzip".equals(code.toLowerCase())
						? new GZIPInputStream(httpurlconnection.getInputStream()) : httpurlconnection.getInputStream();
				byte[] buffer = new byte[1025];
				while ((n = input.read(buffer)) >= 0) {
					out.write(buffer, 0, n);
				}
