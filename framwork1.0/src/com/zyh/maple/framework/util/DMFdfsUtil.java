	...
	public static String uploadFileToFdfs(String filePath, String fileName) throws Exception {
		...
		try {
			try {
				...
				metaList[0] = fileName != null && !"".equals(fileName) ? new NameValuePair("fileName", fileName)
						: new NameValuePair("fileName", filePath.substring(filePath.lastIndexOf("/") + 1));
	...
	public static String uploadFileToFdfs(byte[] file, String fileName) throws Exception {
		...
		try {
			try {
				...
				StorageClient1 client = new StorageClient1(trackerServer, (StorageServer) storageServer);
				NameValuePair[] metaList = new NameValuePair[1];
				metaList[0] = fileName != null ? new NameValuePair("fileName", fileName)
						: new NameValuePair("fileName", "noFileName");
